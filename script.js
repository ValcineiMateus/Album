(function () {
    ...
    // Sobe um único arquivo pro Cloudinary usando um "upload preset" sem
    // assinatura — é o que permite mandar a foto direto do navegador sem
    // precisar de senha nenhuma no código (que ficaria visível pra qualquer
    // um que abrisse o site).
    async function uploadFileToCloudinary(file) {
        ...
        if (!response.ok) {
            throw new Error('Falha ao enviar a foto para o Cloudinary. Confira o cloud name e o upload preset.');
        }
    ...
    // Sobe uma lista de arquivos para o Cloudinary e adiciona as URLs
    // resultantes no campo "images" do dia (documento) já existente no Firestore.
    ...
    async function removeGroup(id) {
        ...
        try {
            // O Cloudinary não permite apagar arquivos com segurança direto do
            // navegador (precisaria de uma senha secreta que não pode aparecer
            // no código) — então removemos só o registro deste dia do banco de
            // dados. As fotos ficam guardadas no Cloudinary, só não aparecem
            // mais aqui no álbum.
