(function () {
    ...
    // ===== SALVAR / CARREGAR =====
    function saveData() {
        localStorage.setItem('albumGroups_v5', JSON.stringify(groups));
    ...
    function loadData() {
        const stored = localStorage.getItem('albumGroups_v5');
        if (stored) {
            try {
                groups = JSON.parse(stored);
                if (groups.length > 0) {
                    let maxId = 0;
                    groups.forEach(g => { if (g.id > maxId) maxId = g.id; });
                    nextId = maxId + 1;
                }
            } catch (e) { groups = []; }
        }
    ...
    function render() {
        ...
        const totalFotos = sorted.reduce((acc, g) => acc + g.images.length, 0);
            ...
            files.forEach(file => {
                ...
                reader.onload = function (ev) {
                    ...
                    if (loaded === files.length) {
                        ...
                        saveData();
                        render();
                        setTimeout(() => openEditModal(newGroup.id), 400);
                    }
                };
    ...
    function addMorePhotos(groupId) {
        ...
        input.onchange = function (e) {
            ...
            files.forEach(file => {
                const reader = new FileReader();
                reader.onload = function (ev) {
                    group.images.push(ev.target.result);
                    loaded++;
                    if (loaded === files.length) {
                        saveData();
                        render();
    ...
    function removeGroup(id) {
        ...
        if (!group) return;
        if (confirm(`Remover este dia (${group.images.length} fotos)?`)) {
    ...
    function saveEdit() {
        ...
        const group = groups.find(g => g.id === currentEditId);
        if (!group) return;

        group.text = editText.value.trim();
        group.date = editDate.value;

        const musicSelect = editMusic.value;
        if (musicSelect === 'custom') {
            group.music = editCustomMusic.value.trim();
        } else if (musicSelect === '') {
            group.music = '';
        } else {
            group.music = musicSelect;
        }
